...
import com.project.zaixianjiaoyu.http.adapter.CacheCall;
import com.project.zaixianjiaoyu.http.adapter.Call;
import com.project.zaixianjiaoyu.http.adapter.CallAdapter;
import com.project.zaixianjiaoyu.http.adapter.DefaultCallAdapter;
...
import com.project.zaixianjiaoyu.http.convert.Converter;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    public BaseRequest(String url) {
        this.url = url;
        baseUrl = url;
        MyOkhttp go = MyOkhttp.getInstance();
    ...
    public RequestBody wrapRequestBody(RequestBody requestBody) {
        ...
        progressRequestBody.setListener(new ProgressRequestBody.Listener() {
            ...
            public void onRequestProgress(final long bytesWritten, final long contentLength, final long networkSpeed) {
                MyOkhttp.getInstance().getDelivery().post(new Runnable() {
                    @Override
                    public void run() {
    ...
    public okhttp3.Call generateCall(Request request) {
        ...
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && interceptors.size() == 0) {
            return MyOkhttp.getInstance().getOkHttpClient().newCall(request);
        } else {
            OkHttpClient.Builder newClientBuilder = MyOkhttp.getInstance().getOkHttpClient().newBuilder();
